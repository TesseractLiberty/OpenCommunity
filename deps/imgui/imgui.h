...
namespace ImGui
{
    ...
    IMGUI_API bool          Button(const char* label, const ImVec2& size = ImVec2(0, 0), const bool hovAnim = true);   // button
    ...
    IMGUI_API bool          Checkbox3(const char* label, bool* v);
    IMGUI_API bool          Checkbox4(const char* label, bool* v);
    ...
    IMGUI_API bool          SliderFloatABC(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0); // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
    ...
    IMGUI_API bool          IsItemTogglefweetn();                                                // was the last item open state toggled? set by TreeNode().
