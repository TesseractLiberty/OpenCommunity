...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const bool hovAnim)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, const bool hovAnim)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, hovAnim);
}
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        return ImRect(outer_rect.Max.x - 5.f, inner_rect.Min.y, outer_rect.Max.x, inner_rect.Max.y);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags)
{
    ...
    const ImS64 win_size_v = ImMax(ImMax(size_contents_v, size_avail_v), 1.f);
    ...
    const ImS64 scroll_max = ImMax(1.f, size_contents_v - size_avail_v);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    SetCursorPos({ 9.f, label_pos.y + 20.f });
    ...
    //hover anim
    {
        ...
        if (label_size.x > 0.0f)
        {
            RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y - 30.f), label);
        }
        ...
        if (actualTime > 0.f) {
            ...
            RenderText(ImVec2((frame_bb.Max.x - frame_bb.Max.x + frame_bb.Min.x) + w - CalcTextSize(value_buf, NULL, true).x - 5.f, frame_bb.Max.y + 25.f - (72.f * easeInOutSine(actualTime))), value_buf);
        }
    }
    ...
    // Draw frame
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2{ 2.f, 0.f }, ImU32(ImColor(ImVec4(22.f / 255.f, 20.f / 255.f, 22.f / 255.f, 1.f))), false, 5.f);
...

    {
        ...
        if (grab_bb.Max.x > grab_bb.Min.x)
            window->DrawList->AddRectFilled(frame_bb.Min + ImVec2{ 2.f, 2.f }, ImVec2{ actualChanging ? actualPos : grab_bb.Max.x - 2.f, frame_bb.Max.y - 2.f }, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 5.f);
    }
...
// Simule um objeto de tema como substituto do 'gui'
struct GuiTheme {
    ImColor text_disabled = ImColor(150, 150, 150);
    ImColor frame_active = ImColor(40, 40, 40);
    ImColor accent_color = ImColor(100, 150, 255);
    ImColor frame_inactive = ImColor(30, 30, 30);
    ImColor border = ImColor(60, 60, 60);
...
bool ImGui::SliderScalar2(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Slider maior: 20px mais � esquerda e 70px mais � direita (total +90px)
    ...
    static std::unordered_map<ImGuiID, float> sliderchange_times;
    static std::unordered_map<ImGuiID, float> sliderchange_poses;
    static std::unordered_map<ImGuiID, float> sliderchange_newPoses;
    static std::unordered_map<ImGuiID, bool> sliderchange_changings;

    if (sliderchange_times.find(id) == sliderchange_times.end())
        sliderchange_times.emplace(id, 0.f);
    if (sliderchange_poses.find(id) == sliderchange_poses.end())
        sliderchange_poses.emplace(id, 0.f);
    if (sliderchange_newPoses.find(id) == sliderchange_newPoses.end())
        sliderchange_newPoses.emplace(id, 0.f);
    if (sliderchange_changings.find(id) == sliderchange_changings.end())
        sliderchange_changings.emplace(id, false);

    float& actualTime = sliderchange_times.at(id);
    float& actualPos = sliderchange_poses.at(id);
    float& actualNewPos = sliderchange_newPoses.at(id);
    bool& actualChanging = sliderchange_changings.at(id);

    constexpr auto easeOutExpo = [](float x)
        {
            return 1.f - pow(1.f - x, 3.f);
        };
    ...
    // Usar cor do background que voc� pediu
    ...
    static std::unordered_map<ImGuiID, float> sliderhover_times;
    if (sliderhover_times.find(id) == sliderhover_times.end())
        sliderhover_times.emplace(id, 0.f);

    float& hoverTime = sliderhover_times.at(id);
    const bool held = (g.ActiveId == id);

    if ((hovered || held) && hoverTime < 1.f)
        hoverTime += 1.f * g.IO.DeltaTime;
    if (!(hovered || held) && hoverTime > 0.f)
        hoverTime -= 1.f * g.IO.DeltaTime;
    ...
    // Texto do valor um pouco mais � direita (5px a mais que antes)
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_SliderGrabActive, color::GetPurpleVec4()); // Cor roxa quando o slider est� ativo

    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);

    ImGui::PopStyleColor(2); // Remove as duas cores que foram adicionadas

}
bool ImGui::SliderFloatABC(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Chama a fun��o SliderScalar com os par�metros fornecidos
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (!is_leaf)
    {
        ...
        if (toggled)
        {
            ...
            g.LastItemData.StatusFlags |= ImGuiItemStatusFlags_Togglefweetn;
        }
    }
    if (flags & ImGuiTreeNodeFlags_AllowItemOverlap)
        SetItemAllowOverlap();
